...
# 定义自定义类型
TYPE_DotsOCRModel = io.Custom(io_type="EASY_DOTSOCR_MODEL")

# 下载并加载模型节点
class DownloadAndLoadDotsOCRModel(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="easy downloadDotsOCRAndLoadModel",
            display_name="DotsOCR Model Loader",
            category="EasyUse/DotsOCR",
            inputs=[
                io.Combo.Input("model_id", options=["rednote-hilab/dots.ocr"], default="rednote-hilab/dots.ocr"),
                io.Combo.Input("download_from", options=["huggingface", "modelscope"], default="huggingface"),
                io.Combo.Input("attention", options=["flash_attention_2", "sdpa", "eager"], default="flash_attention_2"),
                io.AnyType.Input("start", optional=True, tooltip="Start to load or download models"),
            ],
            outputs=[
                TYPE_DotsOCRModel.Output(display_name="dots_ocr_model"),
            ],
    ...
    def execute(cls, model_id: str, download_from: str, attention: str, start: io.AnyType = None) -> io.NodeOutput:
        ...
        model, processor = dotsOCR.load_model(model_path, attn_implementation=attention)
...
# 应用DotsOCR模型进行OCR识别
class ApplyDotsOCR(io.ComfyNode):
    
    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="easy applyDotsOCR",
            display_name="Apply DotsOCR",
            category="EasyUse/DotsOCR",
            inputs=[
                TYPE_DotsOCRModel.Input("dots_ocr_model"),
                io.Image.Input("image", display_name="Input Image"),
                io.Combo.Input("prompt_template", options=PROMPT_TEMPLATE_NAMES, default=PROMPT_TEMPLATE_NAMES[2], tooltip="prompt to guide the OCR process"),
    ...
    @classmethod
    def execute(cls, dots_ocr_model: dict, image: Any, prompt_template: str, unload_model: bool, temperature: float, seed: int, custom_prompt: str = "") -> io.NodeOutput:
        if custom_prompt.strip():
            system_prompt = custom_prompt
        else:
            system_prompt = PROMPT_TEMPLATES_DATA[prompt_template]["system_prompt"]

        result = dotsOCR.infer(dots_ocr_model, images=image, prompt=system_prompt, temperature=temperature, seed=seed)
